...
- 바이너리 파일을 문자 스트림으로 읽으면 읽을 수 없는 바이트가 생겨서 오류발생. 예) 텍스트 파일을 읽는 입력 스트림
바이트 스트림
- 입출력 데이터를 단순 바이트의 흐름으로 처리
- 문자 데이터든 바이너리 데이터든 상관없이 처리가능

## TCP/IP 소개
TCP/IP 프로토콜
- 두시스템 간에 데이터가 손상없이 안전하게 전송되도록 하는 통신 프로토콜
...
- 현재는 32비트의 IP 버전(IPv4)가 사용되고 있음
 - IP주소 고갈로 인해 128비트의 IP버전6이 점점 사용되는 추세
...
멀티스레딩(multi-threading)
 - 여러 스레드를 동시에 실행시키는 응용플고그램을 작성하는 기법
스레드 구성
 - 스레드 코드
...
스레드 동기화
 ...
 - 멀티스레드의 공유 데이터의 동시 접근 문제 해결
   ...
   - 방법2: 한 스레드가 공유 데이터에 대한 작업을 끝낼 때까지 다른 스레디가 대기하도록 함
자바의 스레드 동기화 방법 2가지
...
## synchronized 블록 지정
synchronnized 키워드
 - 스레드가 독점적으로 실행해야하는 부분(동기화 코드)을 표시하는 키워드
...
## 이벤트 기반 프로그래밍
이벤트 기반 프로그래밍
- 이벤트의 방생에 의해 프로그램 흐름이 결정되는 방식
 - 이벤트가 발생하면 이벤트를 처리하는 루틴(이벤트 리스너) 실행
 - 실행될 코든느 이벤트의 발생에 의해 전적으로 결정
...
이벤트와 이벤트 리스너 선택
 - 버튼 클랙을 처리하고자 하는 경우
  - 이벤트 Action 이벤트, 이벤트 리스너 : ActionListner
이벤트 리스너 등록
...
익명 클래스 : 이름 없는 클래스
  ...
  - 간단한 리스너의 경우 익명 클래스 사용 추천
    - 메소드의 개수가 1, 2개인 리스너(ActionListsner, ItemListsner)에 대해 주로 사용
